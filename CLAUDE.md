...
- **篩選（模式化）**：讀 data/screen.json；折舊/償債 ≤n（AND）、欄名升降冪、點列開股、>300 截斷。**scrOpen 時：⚙ 篩選鈕亮金色、個股內容整塊隱藏（`{stock&&!scrOpen&&…}`）、兩顆模式鈕不亮**；點快速瀏覽/完整模式→setScrOpen(false)；openKept（篩選/保留股點列）→先關篩選再 pick。screen.json 由 build_screen 產生，**只收 4 位純數字代號**（基金/受益證券/英數代號 fin 檔排除）
...
4. git 順序：commit → pull --rebase -X theirs → push
