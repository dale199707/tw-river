...
> 交接檔（v14，2026-07-11）。新對話／Claude Code 請先完整讀完本檔再動手。
...
- **保留股（多群組）**：`twri-watch`，每筆 `grps` 陣列（**舊資料 grp 字串自動相容讀取**，grpsOf() 統一入口；調整時自動遷移為 grps）。☆ 彈群組選單加入；★ 彈同一選單切換群組（✓ 標記、可連點多選、含「✕ 取消保留」，**不再一鍵移除**）；面板每列顯示所有群組＋「±群組」切換式下拉（value 固定空字串、選項打 ✓）；群組 chips 計數與「僅在群組間切換」都用 includes 判斷。saveWatch 失敗→清 twri-y 快取重試→再失敗 alert
- **← → 瀏覽**：全市場代號序；「僅在保留股間切換」跟群組篩選；input 聚焦不觸發；背景預抓 [+1,+2,−1]
- **篩選（模式化）**：讀 data/screen.json；折舊/償債 ≤n（AND）、欄名升降冪、點列開股、>300 截斷。**scrOpen 時：⚙ 篩選鈕亮金色、個股內容整塊隱藏、兩顆模式鈕不亮**；點快速瀏覽/完整模式→關閉；openKept→先關面板再 pick。screen.json 由 build_screen 產生，**只收 4 位純數字代號**（基金/受益證券/英數代號 fin 檔排除）
- **盤後選股（pickOpen，與篩選同一套模式化行為、兩者互斥）**：Dale 親自校準的選股＋估價頁。
  **⚠️ 所有定義經 Dale 多輪調校定版（2026-07-10 深夜），未經 Dale 明確要求絕對不可更動任何公式。**
  - 五條件（全過才列入）：revG>0、niG>0、epsG>0、roe>0 且 navG>0、debt∈[0,5)。皆為 screen.json 欄位（build_screen 計算）：成長率＝近4季 vs 前4季（**需連續 8 季可還原單季值、基期須為正**——虧轉盈與資料不足為 null 不列入）；roe＝近4季 ni÷最新 eq；navG＝bvps vs 4 季前（Dale 邏輯：ROE×NAV＝EPS，ROE 穩定下 NAV 成長帶動 EPS）
  - screen.json 估價原料欄：`eps4`（近4季EPS合計）、`epsY`（最近完整年度EPS＝該年 Q4 之 YTD 值）、`peLo/peLo2`（歷年**年度平均本益比**最低/次低，data/price 年度 pe>0）
  - 估價衍生欄（前端 pickRows 計算，現價來自快照故隨收盤更新）：

    | 欄 | 公式 |
    |---|---|
    | 現價 | 快照 close（上市即時、上櫃每日 snap） |
    | 本益比 | 現價 ÷ eps4 |
    | EPS成長率 | eps4 ÷ epsY − 1（%，epsY>0 才算） |
    | 本益成長比 | 本益比 ÷ EPS成長率（成長率>0 才列；<1 便宜、≈1 合理、>2 貴） |
    | 最低/次低PE價 | peLo / peLo2 × eps4 |
    | 12倍PE價 | 12 × eps4 |
    | 便宜價 | eps4 × 3/4 × EPS成長率 |
    | 合理價 | eps4 × 1 × EPS成長率 |

    注意：左段「EPS成長」（epsG，近4季 vs 前4季）**只當選股條件**，不參與估價；估價一律用「EPS成長率」（近4季 vs 去年全年）。歷史演進備查：最低PE 曾改「每年最低價換算」後 Dale 要求改回年度平均；G 曾用 3 年 CAGR 後廢棄；PEG 欄已刪、由本益成長比取代
  - 個股內容閘門 `{stock&&!scrOpen&&!pickOpen&&…}`；screen.json 載入 useEffect 條件為兩面板任一開啟，**依賴陣列必須含 pickOpen**（曾漏加導致「永遠載入中」——改共用 effect 條件時 deps 要同步檢查）
...
- 部署：**Dale 的下載檔一律直接放在 repo 根目錄 `~/Desktop/tw-river-repo`**（不在 ~/Downloads）。給指令時：.py 檔 `mv` 到 pipeline/、.yml 檔 `mv` 到 .github/workflows/、index.html/worker.js/CLAUDE.md 留根目錄 → add（指定檔案）/commit/pull --rebase -X theirs/push。下載檔在根目錄未追蹤沒關係（本專案禁用 git add -A）
...
| 資料 | 頻率 | 機制 | 需要人工？ |
|---|---|---|---|
| 上市當日收盤 | 即時 | Worker /today | 否 |
| 上市快照 PE/PB/殖利率 | 每日 | 前端打 openapi（前一交易日） | 否 |
| 上櫃快照＋收盤＋當年逐月 | 平日 16:40 | tpexsnap.yml → tpex_snap/tpex_ytd | 否 |
| 歷史年價格（雙市場） | 每月 6 日 | pricedata.yml → data/price | 否 |
| 股利（雙市場） | 每季 4/5/8/11 月 16 日 | findata.yml → data/fin 的 div | 否 |
...
**步驟（Dale 自己做或交給 Claude Code 都照此）**：
1. 到公開資訊觀測站 XBRL 資料下載專區，下載該季「金管會證期局 XBRL 財報建檔工具申報檔案」整包
2. 解壓到 `~/Desktop/tw-river-repo/XBRL/tifrs-{YYYY}Q{n}/`（例：`XBRL/tifrs-2026Q2/`，裡面應是數千個 .html 檔）
3. 執行（把 2026Q2 換成實際季別）：
```
...
```
**檢查方式**：
- xbrl_ingest 結尾統計「寫入 N 檔」應約 1900+（上市＋上櫃）；出入清單若列出大量缺漏屬異常
- 抽驗一檔：`python3 -c "import json;d=json.load(open('data/fin/2330.json'));print(sorted(d['q'])[-1])"` 應印出新季別
- push 後等 1–2 分鐘開網站查 2330，「財務指標」分頁最右端應出現新季
**注意**：XBRL/ 目錄在 .gitignore，**絕對不 commit 原始檔**；不可用 git add -A。若某季格式解析大量失敗，參考本檔「xbrl_ingest.py 關鍵知識」節，先 `--quarter ... --dry-run` 比對再處置。

### B. 自動化失敗處理（收到 GitHub Actions 失敗信時）

**tpexsnap（每平日）**
- 單日失敗＝正常（Actions runner 抽到被 TPEX 封的髒 IP），**不用處理**，隔日自癒（斷點續補，資料不會漏）
- 連續 3 天以上失敗：開 Actions log 看「公司清單 state=... detail=...」或 dailyQuotes 錯誤內容。若 detail 是 HTML errors 頁＝IP 問題，手動 Run workflow 重抽 runner 通常即過；其他錯誤帶 log 開新對話
- 手動補跑：GitHub → Actions → tpexsnap → Run workflow（冪等，隨時可按）

**pricedata（每月 6 日）**
- 失敗：先手動 Run workflow 重試一次。log 出現「被擋待重試年度 ≠ 0」＝TPEX/TWSE 限流沒跑完，再 Run 一次續補（斷點＝檔案本身）
- 每年 1 月那次特別留意：要新增前一完結年度，跑完抽驗 `data/price/2330.json` 應含新年度鍵

**findata（每季 16 日）**
- 失敗多半是 MOPS 偶發 502（本專案實際遇過）：手動 Run workflow 重跑即可，merge 冪等
- 跑完網站抽一檔看「股利發放」圖最新期別有沒有出現

### C. 前端偵錯 SOP（症狀 → 檢查 → 處置）

先記住三層快取：**localStorage 快照 `twri-snap2-*`（至多 1 小時）→ localStorage 年資料 `twri-y-{code}-{y}`（歷史年永久、當年當日）→ Worker/瀏覽器 HTTP 快取**。八成「資料怪怪的」清快取就好：DevTools Console 執行
```
...
| 症狀 | 依序檢查 | 處置 |
|---|---|---|
| 上市收盤價不對/停在舊日 | `curl -s https://tw-river-api.dale199707.workers.dev/today` 看 date 與該檔 close | date 舊＝TWSE 未發佈或 Worker 快取，30 分內自癒；Worker 回 error 見下「Worker 偵錯」 |
| 上櫃收盤/快照全掛、股名 — | 開 `https://dale199707.github.io/tw-river/data/tpex_snap.json` 看 date | 檔案舊＝tpexsnap 沒跑成功→照 B 節處理；檔案新＝清 localStorage |
| 上櫃河流圖缺當年 | 開 data/tpex_ytd.json 看 year/last 與該代號 m | 同上；新掛牌股本來就只有當年 |
| 河流圖歷史年空白 | 開 data/price/{code}.json 是否存在/含該年 | 無檔＝pipeline 未涵蓋（新上市，等月更）；有檔仍空＝清 localStorage |
| 財務指標/檢驗圖空白 | 開 data/fin/{code}.json 的 q 鍵 | 無新季＝XBRL 未入庫（A 節）；金融業 inv/ar/ap 空是正常 |
| 篩選表某檔 — | screen.json 該檔 debt/dep 為 null | detail 欄位不足屬正常，null 會被篩選排除 |
| 整頁白畫面 | DevTools Console 看紅字 | Babel 編譯錯＝最近改壞前端，git revert index.html 後重查原因 |

**Worker 偵錯**：worker.js 有全域 try/catch，異常回 JSON `{error,message,stack}`。curl 端點看回應即可定位。改 Worker 流程＝改 repo worker.js → 貼 Cloudflare Edit code → Deploy → curl 驗證 → commit 備份。**Worker 內禁止加任何 TPEX 抓取**（見立即注意事項 2）。

**改前端的鐵律**（弱模型尤其遵守）：抓 repo 最新 index.html → 改動 → 用 @babel/standalone@**7.26.4** 實際編譯驗證＋括號平衡 → 才可交付；改壞的第一動作是 revert 不是硬修。

### D. 給 Claude Code／新對話的執行守則

1. 先完整讀本檔，再動手；不確定就先 --probe / --dry-run，貼輸出等確認
2. 循序處理、不開平行 agent；不做沒被要求的功能；git 指令單一 code block、不加行內註解
3. 只 add 指定檔案（XBRL 原始檔在 repo 內，git add -A 會災難）
4. git 順序：commit → pull --rebase -X theirs → push。**mv 蓋檔後若 git status 出現不在預期清單的 modified，先 git diff 看內容再決定**：新版就 add、舊檔誤蓋就 `git checkout -- 檔名` 還原（2026-07-10 實例：舊版 worker.js 誤蓋，checkout 還原）
5. Dale 的下載檔在 repo 根目錄；.py → pipeline/、.yml → .github/workflows/
6. TPEX 三禁：禁 Worker 代理、禁瀏覽器直連、禁在白天高頻爬（限流知識見專章）
7. 資料檔一律單行 compact JSON（separators=(',',':'), ensure_ascii=False）

## 交接時狀態（2026-07-10 晚，v13）

- ✅ **上櫃（TPEX）前端支援全案完成**（見專章）：公司清單/快照/當日收盤/當年價格/股利雙市場全通並部署驗證（5483 完整模式抽測通過）
- ✅ 前端 UX：篩選模式化（亮起/互斥/隱藏個股/點列跳轉）、保留股多群組（grps 陣列、舊資料相容）、screen.json 只收普通股代號
- ✅ **盤後選股頁全案完成並定版**（2026-07-10 深夜）：五條件選股＋11 估價欄，定義經 Dale 多輪校準（見前端重點章的公式表，**不可擅改**）
...
- ⏳ 待辦（依序）：
  1. **保留股跨裝置同步／匯出匯入**：先做 JSON 匯出/匯入（零後端），同步選項先問使用情境。注意保留股已是 grps 多群組格式
