...
> 交接檔（v15，2026-07-11）。新對話／Claude Code 請先完整讀完本檔再動手。
...
- **盤後選股（pickOpen，與篩選同一套模式化行為、兩者互斥）**：Dale 親自校準的選股＋估價頁。
  **⚠️ 所有定義經 Dale 多輪調校定版（2026-07-11），未經 Dale 明確要求絕對不可更動任何公式或條件。**
  - **四條件**（全過才列入）：`revG>0`（營收成長）、`niG>0`（盈餘成長）、`roe>0`、`debt∈[0,5)`（償債年數<5）。皆為 screen.json 欄位（build_screen 計算）：成長＝近4季 vs 前4季（**需連續 8 季可還原單季值、基期須為正**——虧轉盈與資料不足為 null 不列入）；roe＝近4季 ni÷最新 eq
  - screen.json 另有 `epsG/navG` 欄（已不當條件、不顯示，pipeline 保留計算勿刪）；估價原料欄：`eps4`（近4季EPS合計）、`epsY`（最近完整年度EPS＝該年 Q4 之 YTD 值）、`peLo/peLo2`（歷年**年度平均本益比**最低/次低，data/price 年度 pe>0）
  - 顯示欄（左段）：股號/股名/最新季/營收成長/盈餘成長/ROE/償債年數；估價衍生欄（前端 pickRows 計算，現價來自快照故隨收盤更新，預設排序 EPS成長率降冪）：
    ...
    歷史演進備查（勿走回頭路）：EPS成長與 NAV成長曾為條件後刪除；便宜價/合理價欄曾存在後刪除；最低PE 曾改「每年最低價換算」後改回年度平均；G 曾用 3 年 CAGR 後廢棄；PEG 欄已刪、由本益成長比取代
