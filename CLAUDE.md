...
> 交接檔（v13，2026-07-10）。新對話／Claude Code 請先完整讀完本檔再動手。
...
- **盤後選股（pickOpen，與篩選同一套模式化行為、兩者互斥）**：Dale 的五條件全過清單——營收成長、盈餘成長、EPS 成長、ROE＞0 且 NAV 成長（ROE×NAV＝EPS，ROE 穩定下看 NAV 成長帶動 EPS）、償債年數＜5。資料來自 screen.json 新欄位 `revG/niG/epsG/roe/navG`（build_screen 計算：成長率＝近4季 vs 前4季，**需連續 8 季可還原單季值，基期須為正**——虧轉盈與資料不足者為 null 不列入；roe＝近4季 ni／最新 eq；navG＝bvps vs 4 季前）。個股內容閘門為 `{stock&&!scrOpen&&!pickOpen&&…}`；screen.json 載入條件為兩面板任一開啟
